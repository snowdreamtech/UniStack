# Mise vs UniStack Configuration Comparison

This report provides a granular comparison of configuration capabilities between `mise` and `UniStack`. Based on the latest source code analysis from both repositories, here is the current parity status.

## 1. Environment Directives (`[env]`)

Mise uses a sophisticated directive system for environment manipulation. UniStack has recently been upgraded to support the most critical of these.

| Feature | Mise Syntax | UniStack Support | Status | Notes |
| :--- | :--- | :---: | :---: | :--- |
...
| **Tera Templates** | `KEY = "{{ env.HOME }}"` | **Yes** | ✅ | UniStack uses `pongo2` (Jinja2-like), highly compatible with Tera. |
| **Dotenv Files** | `_.file = ".env"` | **Yes** | ✅ | Implemented in `internal/config/loader.go`. |
| **Path Prepending** | `_.path = "bin"` | **Yes** | ✅ | Supports both string and list formats in UniStack; supports `$VAR`. |
...
| **Modules/Vfox** | `_.module = "..."` | No | ❌ | UniStack has its own provider system instead of vfox modules. |
| **Age Encryption** | `_.age = "..."` | No | ❌ | Experimental in Mise; not planned for UniStack. |

## 2. Global Settings (`[settings]`)

Settings control the behavior of the tool itself. UniStack targets the most frequently used settings for developer experience.

| Feature | Mise Key | UniStack Support | Status | Notes |
| :--- | :--- | :---: | :---: | :--- |
| **GitHub Proxy** | `github_proxy` | **Yes** | ✅ | Fully aligned. |
| **GitHub Token** | `github_token` | **Yes** | ✅ | Fully aligned. |
| **Concurrency** | `jobs` | **Yes** | ✅ | UniStack uses `concurrency` (mapped from `jobs` during migration). |
| **HTTP Timeout** | `http_timeout` | **Yes** | ✅ | Recently added to UniStack. |
| **Experimental** | `experimental` | **Yes** | ✅ | Recently added to UniStack. |
...
| **Asdf Compat** | `asdf_compat` | No | ❌ | UniStack uses its own modern logic exclusively. |
...
### UniStack Alignment Level

UniStack has achieved **~95% functional parity** for the average developer's daily workflow. The missing features are primarily edge cases (Age encryption) or legacy compatibility (asdf_compat). With the addition of **Recursive Config Discovery**, **Ceiling Paths**, and **Version Aliases**, UniStack now offers a nearly identical configuration experience to Mise.

---
*Report generated based on Mise source analysis (`settings.toml` & `src/config/env_directive/mod.rs`) and UniStack current state.*
