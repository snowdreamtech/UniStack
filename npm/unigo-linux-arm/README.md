# UniGo (Universal Runtime Manager)
...
1. **Initialize & Install**: `unigo install` (installs all project tools and dependencies).
...
| `GITHUB_PROXY` | Network optimization (See [Proxy Usage](#-proxy-usage-scenarios)) | `.unigo.toml` |
...
- **Problem**: `unigo install` fails on Windows.
...
- **Problem**: Pre-commit hooks fail on macOS after `unigo install` with Python errors.
  - **Diagnosis**: Check if the venv exists: `ls .venv/bin/python`.
  - **Solution**: Rebuild the venv: `rm -rf .venv && unigo install`.
...
- [Full Documentation](https://github.com/snowdreamtech/UniGo/tree/main/docs/index.md)
- [Project Glossary](https://github.com/snowdreamtech/UniGo/tree/main/docs/glossary.md)
