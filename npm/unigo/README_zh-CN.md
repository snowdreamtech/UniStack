...
1. **初始化与安装**：`unigo install`（安装开发所需核心工具与项目依赖）。
...
| `GITHUB_PROXY` | 网络优化 (见 [代理使用场](#-代理使用场景)) | `.unigo.toml` |
...
- **问题**: `unigo install` 在 Windows 上失败。
...
- **问题**: `unigo install` 后，macOS 上的 Pre-commit 钩子出现 Python 报错。
  - **诊断**: 检查 venv 是否存在：`ls .venv/bin/python`。
  - **解决方案**: 重建 venv：`rm -rf .venv && unigo install`。
...
- [完整文档](https://github.com/snowdreamtech/UniGo/tree/main/docs/index.md)
- [项目术语表](https://github.com/snowdreamtech/UniGo/tree/main/docs/glossary.md)
