...
Every script MUST be safely re-executable without side effects or errors. Running `unistack run setup` ten
...
- **Reason rejected:** Requires Python or Node.js to be pre-installed before `unistack run setup` can run,
  creating a bootstrapping paradox. POSIX shell is universally available on all target platforms and
  has zero external dependencies.

### Option C: .unistack.toml-only Automation

Express all automation as .unistack.toml targets with no shell scripts.
...
- **Reason rejected:** GNU Make has subtle cross-platform differences, particularly regarding
  ...
  .unistack.toml syntax. A hybrid approach (`.unistack.toml` targets invoke `scripts/*.sh`) is used instead.
