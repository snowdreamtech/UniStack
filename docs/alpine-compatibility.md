...
```bash
ldd $(unistack where python)/bin/python3
# Alpine 上会显示 musl 依赖
```
...
```bash
# 配置 unistack 使用非官方构建
unistack settings set node.mirror_url=https://unofficial-builds.nodejs.org/download/release/
unistack settings set node.flavor=musl
```
...
# unistack 会自动从源码编译（较慢）
unistack settings set node.compile=true
```
...
RUN unistack settings set node.mirror_url=https://unofficial-builds.nodejs.org/download/release/ && \
    unistack settings set node.flavor=musl
...
**A**: 默认情况下，unistack 尝试下载 glibc 版本的 Node.js，不兼容 Alpine。
...
**A**: 不需要。unistack 会自动检测 musl 并下载正确的版本。

### Q3: 应该使用 Alpine 官方包还是 unistack？

**A**:

- **生产环境**: 推荐使用 Alpine 官方包（更稳定、更小）
- **开发环境**: 可以使用 unistack（版本管理更灵活）
...
```bash
# 检查二进制依赖
ldd $(unistack where node)/bin/node
# 应该显示 musl 相关的库

# 或检查文件类型
file $(unistack where node)/bin/node
...
1. **Go**: 直接使用 unistack，无需特殊配置
2. **Python**: 直接使用 unistack，自动处理 musl
3. **Node.js**: 配置使用 musl 构建或使用 Alpine 官方包
4. **生产环境**: 优先使用 Alpine 官方包
5. **开发环境**: 可以使用 unistack + musl 构建
...
- [unistack Node.js 文档](https://github.com/snowdreamtech/UniStacklang/node.html)
