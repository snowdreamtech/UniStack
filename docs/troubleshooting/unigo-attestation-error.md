...
在运行 `make sync-lock` 或 `unigo install` 时，可能会遇到以下错误：
...
UNIRTM_VERBOSE=1 unigo install github:astral-sh/ruff@0.15.10 2>&1 | grep -i "aqua\|github"
...
```bash
unigo install github:astral-sh/ruff@0.15.10 --yes
```
