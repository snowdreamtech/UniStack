# Troubleshooting UniStack Provenance Verification Issues

## Problem

When running `make sync-lock` or `unistack lock`, you may encounter an error like:

```
unistack ERROR github:astral-sh/ruff@0.15.9 has no provenance verification on macos-x64,
...
The issue is caused by stale cached metadata in unistack's cache directory. When unistack checks for provenance verification:

1. It caches the verification status of previous versions (e.g., ruff 0.15.8 had attestations)
2. When a new version is released (e.g., ruff 0.15.9), unistack may fail to fetch fresh attestation data
3. The cached metadata becomes outdated, causing false positives for supply chain attacks

## Solution

Clear the unistack cache before synchronizing the lockfile. This has been implemented in `scripts/sync-lock.sh`:

```sh
# Clear unistack cache to avoid stale provenance verification data
log_debug "Clearing unistack cache to refresh provenance verification data..."
unistack cache clear >/dev/null 2>&1 || true
```
...
- UniStack cache documentation: <https://github.com/snowdreamtech/UniStackcli/cache.html>
...
- [UniStack Cache Clear Discussion](https://github.com/jdx/unistack/discussions/7267)
