...
```bash
# 仅在单个命令中模拟 CI
CI=true GITHUB_ACTIONS=true unistack run verify

# 或使用 env 命令
env CI=true GITHUB_ACTIONS=true unistack run audit
```
...
```bash
# 本地模式运行
unistack run verify > local.log 2>&1

# CI 模式运行
source scripts/simulate-ci.sh
unistack run verify > ci.log 2>&1
...
- 工具是否在 `.unistack.toml` 中正确配置
- 工具版本是否与 `scripts/lib/versions.sh` 一致
- unistack 缓存是否需要刷新：`unistack cache clear`
...
1. **提交前测试**: 在提交代码前，使用 CI 模式运行一次完整测试
   ...
   ```bash
   source scripts/simulate-ci.sh
   unistack run verify && unistack run audit
   ```
